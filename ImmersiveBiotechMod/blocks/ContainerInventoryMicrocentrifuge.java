 ...
 * for use in guis
 */
public class ContainerInventoryMicrocentrifuge extends Container {

	// Stores the tile entity instance for later use
	private TileInventoryMicrocentrifuge tileInventoryMicrocentrifuge;
	...
	public ContainerInventoryMicrocentrifuge(InventoryPlayer invPlayer, TileInventoryMicrocentrifuge tileInventoryMicrocentrifuge) {
		this.tileInventoryMicrocentrifuge = tileInventoryMicrocentrifuge;
		...
		for (int x = 0; x < FUEL_SLOTS_COUNT; x++) {
			...
			addSlotToContainer(new SlotFuel(tileInventoryMicrocentrifuge, slotNumber, FUEL_SLOTS_XPOS + SLOT_X_SPACING * x, FUEL_SLOTS_YPOS));
		}
		...
		for (int y = 0; y < INPUT_SLOTS_COUNT; y++) {
			...
			addSlotToContainer(new SlotSmeltableInput(tileInventoryMicrocentrifuge, slotNumber, INPUT_SLOTS_XPOS, INPUT_SLOTS_YPOS+ SLOT_Y_SPACING * y));
		}
		...
		for (int y = 0; y < OUTPUT_SLOTS_COUNT; y++) {
			...
			addSlotToContainer(new SlotOutput(tileInventoryMicrocentrifuge, slotNumber, OUTPUT_SLOTS_XPOS, OUTPUT_SLOTS_YPOS + SLOT_Y_SPACING * y));
		}
	}
	...
	{
		return tileInventoryMicrocentrifuge.isUsableByPlayer(player);
	}
	...
	public ItemStack transferStackInSlot(EntityPlayer player, int sourceSlotIndex)
	{
		...
		if (sourceSlotIndex >= VANILLA_FIRST_SLOT_INDEX && sourceSlotIndex < VANILLA_FIRST_SLOT_INDEX + VANILLA_SLOT_COUNT) {
			...
			if (!TileInventoryMicrocentrifuge.getSmeltingResultForItem(sourceStack).isEmpty()){  //isEmptyItem
				if (!mergeItemStack(sourceStack, FIRST_INPUT_SLOT_INDEX, FIRST_INPUT_SLOT_INDEX + INPUT_SLOTS_COUNT, false)){
					return ItemStack.EMPTY;  //EMPTY_ITEM;
				}
			}	else if (TileInventoryMicrocentrifuge.getItemBurnTime(sourceStack) > 0) {
	...
	public void detectAndSendChanges() {
		...
		boolean fieldHasChanged [] = new boolean[tileInventoryMicrocentrifuge.getFieldCount()];
		if (cachedFields == null) {
			cachedFields = new int[tileInventoryMicrocentrifuge.getFieldCount()];
			allFieldsHaveChanged = true;
		}
		for (int i = 0; i < cachedFields.length; ++i) {
			if (allFieldsHaveChanged || cachedFields[i] != tileInventoryMicrocentrifuge.getField(i)) {
				cachedFields[i] = tileInventoryMicrocentrifuge.getField(i);
    ...
    for (IContainerListener listener : this.listeners) {
			for (int fieldID = 0; fieldID < tileInventoryMicrocentrifuge.getFieldCount(); ++fieldID) {
	...
	public void updateProgressBar(int id, int data) {
		tileInventoryMicrocentrifuge.setField(id, data);
	}
	...
	public class SlotFuel extends Slot {
		...
		public boolean isItemValid(ItemStack stack) {
			return TileInventoryMicrocentrifuge.isItemValidForFuelSlot(stack);
		}
	}
	...
	public class SlotSmeltableInput extends Slot {
		...
		@Override
		public boolean isItemValid(ItemStack stack) {
			return TileInventoryMicrocentrifuge.isItemValidForInputSlot(stack);
		}
	}
	...
	public class SlotOutput extends Slot {
		...
		@Override
		public boolean isItemValid(ItemStack stack) {
			return TileInventoryMicrocentrifuge.isItemValidForOutputSlot(stack);
		}
	}
}
